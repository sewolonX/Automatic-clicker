...
// @version      4.4
// @description  选取元素后自动点击/输入，支持多选、队列模式，跨刷新保存，初始最小化。
// @author       GLM
// @match        *://*/*
// @grant        none
...
(function () {
    ...
    style.textContent = `
        :root {
            --panel-bg: #1a1a2e; --panel-border: #333; --panel-text: #e0e0e0;
            --panel-input-bg: #0f0f23; --panel-input-border: #333; --panel-input-text: #e0e0e0;
            --panel-label-text: #888; --panel-button-bg: rgba(255,255,255,0.06);
            --panel-button-border: rgba(255,255,255,0.1); --panel-button-text: #999;
            --panel-button-hover-bg: rgba(255,255,255,0.12); --panel-button-hover-text: #fff;
            --panel-active-border: #4ade80; --panel-active-text: #4ade80;
            --panel-waiting-text: #f59e0b; --panel-highlight: #f59e0b;
            --panel-selected-highlight: #4ade80; --panel-missing-border: #dc2626; --panel-missing-text: #dc2626;
        }
        [data-theme="light"] {
            --panel-bg: #ffffff; --panel-border: #e5e7eb; --panel-text: #1f2937;
            --panel-input-bg: #f9fafb; --panel-input-border: #d1d5db; --panel-input-text: #1f2937;
            --panel-label-text: #6b7280; --panel-button-bg: rgba(0,0,0,0.05);
            --panel-button-border: rgba(0,0,0,0.1); --panel-button-text: #6b7280;
            --panel-button-hover-bg: rgba(0,0,0,0.1); --panel-button-hover-text: #1f2937;
            --panel-active-border: #10b981; --panel-active-text: #10b981;
            --panel-waiting-text: #d97706; --panel-highlight: #d97706;
            --panel-selected-highlight: #10b981; --panel-missing-border: #dc2626; --panel-missing-text: #dc2626;
        ...
        #ac-panel {
            ...
            border-radius: 12px; padding: 0; width: 300px; font-size: 13px;
            box-shadow: 0 8px 32px rgba(0,0,0,0.5); transition: opacity 0.3s;
            overflow: hidden; max-height: 570px; overflow-y: auto;
        }
        .ac-header {
            display: flex; align-items: center; justify-content: space-between;
            padding: 14px 12px 10px 18px; cursor: move; min-height: 44px; touch-action: none;
        }
        .ac-header h3 {
            margin: 0; font-size: 15px; font-weight: 700; color: var(--panel-text);
        ...
        .ac-toggle {
            ...
            padding: 0; border-radius: 6px; margin-left: 8px; line-height: 1; transition: all 0.2s;
        }
        ...
        .ac-body { padding: 0 18px 18px; }
        #ac-panel.collapsed .ac-body { display: none; }
        #ac-panel.collapsed { width: auto; }
        .ac-row { margin-bottom: 12px; }
        .ac-row label { display: block; font-size: 11px; color: var(--panel-label-text); margin-bottom: 5px; letter-spacing: 0.5px; }
        ...
        .ac-row input[type="number"]:focus, .ac-row select:focus, .ac-row input[type="text"]:focus { border-color: var(--panel-active-border); }
        ...
        .ac-row-checkbox { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
        .ac-row-checkbox label { margin-bottom: 0; flex: 1; font-size: 11px; color: var(--panel-label-text); letter-spacing: 0.5px; }
        .ac-row-checkbox input[type="checkbox"] {
            width: 16px; height: 16px; cursor: pointer; color-scheme: dark; accent-color: var(--panel-active-border);
            background-color: var(--panel-input-bg); border: 1px solid var(--panel-input-border);
        }
        .ac-target-list-container { min-height: 34px; }
        .ac-target-info {
            background: var(--panel-input-bg); border: 1px solid var(--panel-input-border);
            border-radius: 6px; padding: 8px 10px; font-size: 12px; color: var(--panel-label-text);
            word-break: break-all; min-height: 34px; line-height: 1.5;
        }
        .ac-target-list { max-height: 200px; overflow-y: auto; display: flex; flex-direction: column; gap: 8px; }
        .ac-target-item {
            background: var(--panel-input-bg); border: 1px solid var(--panel-input-border);
            border-radius: 6px; padding: 8px 10px; font-size: 12px; color: var(--panel-label-text);
            word-break: break-all; line-height: 1.5; position: relative; min-height: 52px;
            max-height: 60px; overflow-y: auto; box-sizing: border-box; transition: border-color 0s, color 0s;
        }
        ...
        .ac-target-item span { display: block; padding-right: 20px; white-space: pre-wrap; }
        .ac-btn-item-del {
            position: absolute; top: 4px; right: 4px; width: 16px; height: 16px;
            background: var(--panel-button-bg); border: 1px solid var(--panel-button-border);
            color: var(--panel-button-text); font-size: 10px; line-height: 14px; text-align: center;
            border-radius: 4px; cursor: pointer; padding: 0; transition: all 0.2s;
        }
        .ac-btn-item-del:hover { background: #dc2626; color: #fff; border-color: #dc2626; }
        .ac-btn-group { display: flex; gap: 8px; margin-top: 14px; }
        .ac-btn { flex: 1; padding: 9px 0; border: none; border-radius: 6px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s; }
        ...
        .ac-btn-pick.picking { background: #f59e0b; color: #000; animation: ac-pulse 1s infinite; }
        ...
        .ac-status {
            margin-top: 12px; padding-top: 12px; border-top: 1px solid #222;
            font-size: 12px; color: var(--panel-label-text); display: flex; justify-content: space-between; align-items: center;
        }
        .ac-status .ac-count { color: var(--panel-active-text); font-weight: 700; font-size: 14px; }
        .ac-status.running .ac-count { animation: ac-pulse 0.8s infinite; }
        .ac-status .ac-waiting { color: var(--panel-waiting-text); font-size: 11px; }
        ...
        .ac-btn-cancel {
            flex-shrink: 0; padding: 1px 8px; font-size: 11px; background: var(--panel-button-bg);
            border: 1px solid var(--panel-button-border); color: var(--panel-button-text);
            border-radius: 4px; cursor: pointer; white-space: nowrap; max-width: 40px; text-align: center;
        }
    ...
    function matchesFingerprint(el, fp, matchMode) {
        ...
        if (matchMode === 'strict') {
            ...
            if (fp.text) {
                const elText = getElText(el);
                if (elText !== fp.text && !fp.hasStrong) return false;
            }
        } else {
            // 宽松模式：快速文本校验（防止频繁触发 getComputedStyle 导致页面卡顿）
            if (fp.text) {
                ...
                if (!elText) {
                    ...
                    for (const attr of visualAttrs) {
                        const val = el.getAttribute(attr);
                        if (val && val.trim()) { elText = val.trim(); break; }
                    }
                }
                if (!elText) elText = getElText(el); // 轻量提取失败，才走完整提取
    ...
    function tryFindTarget(targetObj) {
        ...
        try {
            if (targetObj.strict) { const found = verifyList(document.querySelectorAll(targetObj.strict)); if (found) return found; }
            if (targetObj.loose) { const found = verifyList(document.querySelectorAll(targetObj.loose)); if (found) return found; }
    ...
    function saveData() {
        const toSave = {
            ...
            maxClicks: maxClicksInput.value, missingAction: missingActionSelect.value,
            autoFillContent: autoFillInput.value,
            targets: targets.map(t => ({ strict: t.strict, loose: t.loose, fingerprint: t.fingerprint, desc: t.desc, isInput: t.isInput, matchMode: t.matchMode }))
        };
    ...
    function loadData() {
        ...
        try {
            ...
            isMultiMode = cfg.isMultiMode || false; multiModeCheckbox.checked = isMultiMode;
            strategyRow.style.display = isMultiMode ? 'block' : 'none';
            clickStrategy = cfg.clickStrategy || 'simultaneous'; strategySelect.value = clickStrategy;
            clickInterval = cfg.clickInterval || 1100; clickIntervalInput.value = clickInterval;
            maxClicksInput.value = cfg.maxClicks || '';
            missingActionSelect.value = cfg.missingAction || 'wait';
            autoFillContent = cfg.autoFillContent || ''; autoFillInput.value = autoFillContent;
            targets = (cfg.targets || []).map(t => {
                const obj = { strict: t.strict, loose: t.loose, fingerprint: t.fingerprint, desc: t.desc, isInput: !!t.isInput, matchMode: t.matchMode || 'strict', element: null };
                const found = tryFindTarget(obj);
                if (found) { obj.element = found; found.classList.add('ac-selected-highlight'); }
                return obj;
            });
            updateTargetUI();
        } catch (e) { console.error("自动操作脚本配置加载失败", e); }
    }
    ...
    function updateTargetUI() {
        if (targets.length === 0) {
            ...
            btnClearAll.style.display = 'none'; btnStart.disabled = true; return;
        }
        btnClearAll.style.display = 'inline-block'; btnStart.disabled = false;
    ...
    function onDragStart(e) {
        ...
        isDragging = true; const pos = getEventPos(e); const rect = panel.getBoundingClientRect();
        dragOffX = pos.x - rect.left; dragOffY = pos.y - rect.top; e.preventDefault();
    }
    function onDragMove(e) {
        if (!isDragging) return; const pos = getEventPos(e);
        let x = Math.max(0, Math.min(pos.x - dragOffX, window.innerWidth - panel.offsetWidth));
        let y = Math.max(0, Math.min(pos.y - dragOffY, window.innerHeight - panel.offsetHeight));
        panel.style.left = x + 'px'; panel.style.top = y + 'px'; panel.style.right = 'auto'; e.preventDefault();
    }
    ...
    toggleBtn.addEventListener('click', (e) => {
        e.stopPropagation(); panel.classList.toggle('collapsed');
        toggleBtn.textContent = panel.classList.contains('collapsed') ? '+' : '−';
    });

    // ========== 交互事件 ==========
    multiModeCheckbox.addEventListener('change', (e) => {
        isMultiMode = e.target.checked; strategyRow.style.display = isMultiMode ? 'block' : 'none';
        clickStrategy = strategySelect.value; clearSelection(); saveData();
    });
    ...
    btnPick.addEventListener('click', (e) => {
        ...
        isPicking = !isPicking;
        if (isPicking) {
            btnPick.textContent = '取消选取'; btnPick.classList.add('picking');
    ...
    function onPickTouch(e) {
        if (!isPicking || isDragging) return; const touch = e.changedTouches[0];
        const el = document.elementFromPoint(touch.clientX, touch.clientY);
        if (!el || panel.contains(el)) return; e.preventDefault(); e.stopPropagation(); selectTarget(el);
    }
    function onPickClick(e) {
        if (!isPicking) return; const el = e.target;
        if (panel.contains(el)) return; e.preventDefault(); e.stopPropagation(); selectTarget(el);
    }

    function selectTarget(el) {
        el.classList.remove('ac-highlight');
        if (targets.some(t => t.element === el) && isMultiMode) { exitPickMode(); return; }

        const sels = buildSelectors(el); const fp = getElementFingerprint(el);
        ...
        const targetObj = { element: el, strict: sels.strict, loose: sels.loose, fingerprint: fp, desc, isInput, matchMode: 'strict' };
        if (isMultiMode) {
            targets.push(targetObj); el.classList.add('ac-selected-highlight');
            ...
            targets = [targetObj]; el.classList.add('ac-selected-highlight');
            exitPickMode(); stateSpan.textContent = '就绪';
        }
        updateTargetUI(); saveData();
    }
    ...
    function clearSelection() {
        targets.forEach(t => { if (t.element) t.element.classList.remove('ac-selected-highlight'); });
        targets = []; currentQueueIndex = 0; updateTargetUI(); saveData();
    }
    btnClearAll.addEventListener('click', (e) => { e.stopPropagation(); clearSelection(); });

    btnStart.addEventListener('click', (e) => {
        e.stopPropagation(); if (targets.length === 0) return;
        if (!isRunning) startClicking();
        else { stateSpan.textContent = '已停止'; stopClicking(); } // 纯净的手动停止逻辑
    });
    ...
    function startClicking() {
        targets.forEach(t => {
            if (!document.contains(t.element)) {
                ...
                if (found) { t.element = found; found.classList.add('ac-selected-highlight'); }
            }
        });
        const intervalValue = clickIntervalInput.value.trim();
        clickInterval = intervalValue ? parseInt(intervalValue, 10) : 1100;
        isRunning = true; clickedCount = 0; currentQueueIndex = 0; countSpan.textContent = '0';
        const val = maxClicksInput.value.trim();
        maxClicks = val === '' ? Infinity : parseInt(val, 10) || Infinity;
        btnStart.textContent = '停止'; btnStart.className = 'ac-btn ac-btn-stop';
        btnPick.disabled = true; multiModeCheckbox.disabled = true; strategySelect.disabled = true;
        maxClicksInput.disabled = true; clickIntervalInput.disabled = true;
        missingActionSelect.disabled = true; autoFillInput.disabled = true;
        statusDiv.classList.add('running'); stateSpan.textContent = '运行中'; stateSpan.classList.remove('ac-waiting');
        doClick(); timerID = setInterval(doClick, clickInterval); saveData();
    }
    ...
    function doClick() {
        ...
        const status = targets.map((t, i) => {
            ...
            if (!isValid) {
                ...
                if (el) {
                    ...
                    t.element = el; el.classList.add('ac-selected-highlight'); isValid = true;
                }
            }
            updateTargetItemStyle(i, !isValid); return isValid;
        });
            ...
            if (status[idx]) {
                ...
                if (t.isInput && autoFillContent) {
                    if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
                        el.value = autoFillContent; el.dispatchEvent(new Event('input', { bubbles: true })); el.dispatchEvent(new Event('change', { bubbles: true }));
                    } else if (el.isContentEditable) { el.innerHTML = autoFillContent; }
                } else { el.click(); }
                clickedCount++; countSpan.textContent = clickedCount;
                stateSpan.textContent = `队列[${idx + 1}/${targets.length}]`; stateSpan.classList.remove('ac-waiting');
                ...
                if (missingActionSelect.value === 'stop') {
                    stopClicking(); stateSpan.textContent = `队列[${idx + 1}] 元素已消失`; stateSpan.classList.remove('ac-waiting');
                } else {
                    stateSpan.textContent = `队列[${idx + 1}] 等待元素中`; stateSpan.classList.add('ac-waiting');
                }
            }
            return;
        }
        ...
        // 3. 同时点击模式
        let shouldStop = false; let anyClicked = false;
        targets.forEach((t, i) => {
            if (status[i]) {
                anyClicked = true; const el = t.element;
                if (t.isInput && autoFillContent) {
                    if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
                        el.value = autoFillContent; el.dispatchEvent(new Event('input', { bubbles: true })); el.dispatchEvent(new Event('change', { bubbles: true }));
                    } else if (el.isContentEditable) { el.innerHTML = autoFillContent; }
        ...
        if (shouldStop) { stopClicking(); stateSpan.textContent = '元素已消失'; stateSpan.classList.remove('ac-waiting'); return; }
        if (anyClicked) {
            clickedCount++; countSpan.textContent = clickedCount;
            stateSpan.textContent = '运行中'; stateSpan.classList.remove('ac-waiting');
            if (clickedCount >= maxClicks) { stopClicking(); stateSpan.textContent = '已完成'; }
        }
    }
    ...
    function stopClicking() {
        ...
        statusDiv.classList.remove('running'); stateSpan.classList.remove('ac-waiting');
    }
