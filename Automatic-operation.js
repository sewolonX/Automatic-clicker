// ==UserScript==
// @name         Automatic-operation (Persistent)
// @namespace    https://github.com/sewolonX/Automatic-operation
// @version      4.2
// @description  选取元素后自动点击/输入，支持多选、队列、匹配模式，跨刷新保存，初始最小化。
// @author       GLM
// @match        *://*/*
// @grant        none
...
(function () {
    ...
    style.textContent = `
        ...
        #ac-panel {
            ...
            padding: 0;
            width: 300px;
            font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif, "MiSans";
            font-size: 13px;
        ...
        .ac-row input[type="number"],
        .ac-row select,
        .ac-row input[type="text"] {
            width: 100%;
        ...
        .ac-row input[type="number"]:focus,
        .ac-row select:focus,
        .ac-row input[type="text"]:focus {
    ...
    panel.innerHTML = `
        ...
        <div class="ac-body">
            ...
            <div class="ac-row">
                <label>自动填充内容</label>
                <input type="text" id="ac-auto-fill" placeholder="输入内容">
            </div>
            <div class="ac-row">
                <label>点击次数（留空为无限）</label>
                <input type="number" id="ac-max-clicks" min="0" placeholder="无限次">
            </div>
            <div class="ac-row">
                <label>点击间隔（ms）</label>
    ...
    function updateTargetUI() {
        ...
        targets.forEach((t, i) => {
            html += `<div class="ac-target-item active">
    ...
    function selectTarget(el) {
        ...
        if (targets.some(t => t.element === el)) {
            if (!isMultiMode) {} else {
                exitPickMode();
                return;
            }
    ...
    function doClick() {
        ...
        if (isMultiMode && clickStrategy === 'sequential') {
            ...
            while (looped < targets.length) {
                const t = targets[currentQueueIndex];
                let el = t.element;
                ...
                if (isValid) {
                    ...
                    clickedCount++;
                    hasClicked = true;
                    stateSpan.textContent = `队列[${currentQueueIndex + 1}/${targets.length}]`;
                    stateSpan.classList.remove('ac-waiting');
                    currentQueueIndex = (currentQueueIndex + 1) % targets.length;
                    break;
                } else {
                    if (missingActionSelect.value === 'stop') {
                        stopClicking();
                        stateSpan.textContent = `队列[${currentQueueIndex + 1}] 元素已消失`;
                        stateSpan.classList.remove('ac-waiting');
                        return;
                    } else {
                        stateSpan.textContent = `队列[${currentQueueIndex + 1}] 等待中...`;
            ...
            if(!hasClicked && missingActionSelect.value !== 'stop') {
                stateSpan.textContent = '队列为空';
                return;
            }
        ...
        countSpan.textContent = clickedCount;
        if (stateSpan.textContent.includes('等待')) return;
        stateSpan.textContent = '运行中';
        stateSpan.classList.remove('ac-waiting');
        if (clickedCount >= maxClicks) {
            stopClicking();
    ...
    function stopClicking() {
        ...
        stateSpan.classList.remove('ac-waiting');
        if (stateSpan.textContent === '运行中') stateSpan.textContent = '已停止';
    }
