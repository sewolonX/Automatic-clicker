...
// @version 3.2
// @description 选取页面元素后，以1050ms间隔自动连续点击，层级+指纹双重验证，支持不同item区分
// @author You
// @match *://*/*
// @grant none
...
(function () {
    ...
    style.textContent = `
        ...
        .ac-btn-pick:disabled,
        .ac-btn-start:disabled {
            opacity: 0.4;
            cursor: not-allowed;
        }
        .ac-btn-start {
    ...
    panel.innerHTML = `
        ...
        <div class="ac-body">
            <div class="ac-row">
                <label>目标元素</label>
                <div class="ac-target-info" id="ac-target-info">未选择，请点击下方按钮选取</div>
            </div>
            <div class="ac-row">
            ...
            <div class="ac-row" style="margin-bottom:0; font-size:12px; color:#555;">
                间隔：1050ms / 次
            </div>
    ...
    function getElementFingerprint(el) {
        ...
        return {
            ...
            dataAttrs: dataAttrs,
            attrs: attrs,
            onclickParam: onclickParam,  // 新增：onclick中的参数
    ...
    function matchesFingerprint(el, fp) {
        ...
        // 文本匹配（有强特征时文本变了也能放过）
        if (fp.text) {
            const elText = (el.textContent || '').trim().slice(0, 80);
    ...
    function selectTarget(el) {
        ...
        const text = el.textContent ? el.textContent.slice(0, 30).trim() : '';
        if (text) desc += ' "' + text + '"';
    ...
    function startClicking() {
        ...
        stateSpan.classList.remove('ac-waiting');
        doClick();
        timerID = setInterval(doClick, 1050);
    }
